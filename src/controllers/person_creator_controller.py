...
class PersonCreatorController:
    ...
    def create(self, person_info: Dict) -> Dict: # type: ignore
    ...
    def __validate_first_and_last_name(self, first_name: str, last_name:str) -> None:
        #makes sure that characters that are not letters cannot be put on names.
        non_valid_character = re.compile(r'[^a-zA-Z]')

        if non_valid_character.search(first_name) or non_valid_character.search(last_name):
    ...
    def __format_response(self, person_info:Dict) -> Dict: # type: ignore
        return{
            "data":{
                "type": "Person",
                "count": 1,
